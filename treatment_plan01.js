// ==================================================================================================
// === SCRIPT DE TRAITEMENT FINAL (PUPPETEER) - v12 "CIBLAGE DU 2ÈME BOUTON"                      ===
// === Modifie le Plan C pour ignorer le premier bouton (/show) et cibler spécifiquement le       ===
// === deuxième bouton (/confirm) dans la ligne du tableau.                                     ===
// ==================================================================================================
...
async function runTreatment(idSaisi, codeSaisi, userMRA, mdpMRA, logCallback) {
    ...
    try {
        ...
        if (!page.url().includes(verificationUrlPart)) { throw new Error(`Le filtrage a échoué (Plan A et B).`); }
        log(`--- FIN SÉQUENCE 2 : Appareil ${idSaisi} trouvé. ---\n`);

        // ================================================================================
        // === SÉQUENCE 3/4 : SAISIE DU CODE (PLAN C CORRIGÉ POUR CIBLER LE 2ÈME BOUTON) ===
        // ================================================================================
        log("--- SÉQUENCE 3 : CONFIRMATION DU TRAITEMENT (RENFORCÉE v3) ---");
        ...
        try {
            ...
            await page.waitForSelector(selectorA, { visible: true, timeout: 10000 });
        ...
        // PLAN B: Sélecteur XPath complexe
        if (!clickSuccess) {
            try {
                log(`[3.2] PLAN B : Tentative de clic avec le sélecteur XPath complexe.`);
                const selectorB = `//tr[.//td[contains(.,'${idSaisi}')]]//a[@aria-label='Confirmer']`;
                const [button] = await page.$x(selectorB);
                if (button) { await button.click(); clickSuccess = true; log("[3.2] SUCCÈS (PLAN B) : Clic réussi."); }
        ...
        // PLAN C: Exécution JS pour forcer le clic sur le DEUXIÈME bouton
        if (!clickSuccess) {
            try {
                log("[3.2] PLAN C (v3) : Exécution de JS pour forcer le clic sur le DEUXIÈME bouton de la ligne.");
                const clicked = await page.evaluate((id) => {
                    const rows = Array.from(document.querySelectorAll('tr'));
                    const targetRow = rows.find(row => row.innerText.includes(id));
                    if (targetRow) {
                        // On récupère TOUS les liens dans la ligne
                        const links = targetRow.querySelectorAll('a[href*="#/device/"]');
                        // Le premier lien (index 0) est "show", le deuxième (index 1) est "confirm"
                        if (links.length > 1) {
                            links[1].click(); // ON CLIQUE SUR LE DEUXIÈME
                            return true;
                        }
                    }
                    return false;
                }, idSaisi);
                if (!clicked) { throw new Error("Le script d'exécution forcée n'a pas trouvé le 2ème bouton."); }
                clickSuccess = true;
                log("[3.2] SUCCÈS (PLAN C) : Le script a trouvé et cliqué sur le deuxième bouton.");
            } catch (e3) {
                 throw new Error(`ÉCHEC TOTAL : Le clic a échoué avec les 3 méthodes. Erreur finale: ${e3.message}`);
            }
        }
